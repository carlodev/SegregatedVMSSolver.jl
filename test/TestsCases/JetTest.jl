...
function SegregatedVMSSolver.CreateProblem.create_boundary_conditions(simcase::Jet, u_free,u_SEM, u_wall) 
        u_diri_tags=["jet_inlet"]
        u_diri_values = [u_SEM]
        p_diri_tags=["outer"]
        p_diri_values = [0.0]
...
airfoil_mesh_file = joinpath(@__DIR__,"..", "..", "models", "Jet_coarse.msh")
...
physicalp = PhysicalParameters(Re=Re, u_in=u_in)

#Turbulence
Vboxinfo = VirtualBox((-0.5, 0.5), (-0.5,0.5); σ=0.05)
...
mcase = Jet(meshp,simparams,sprob)
...
#mpiexecjl -n 4 julia --project=. test/TestsCases/JetTest.jl

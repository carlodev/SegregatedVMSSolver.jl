...
function segregated_equations(u_adv,params)
    @unpack ν,dΩ, dt, θ, prob = params
    @unpack skew = prob
    skewcoeff = skew * 0.5 # ==0 if skew == false
    vms_activation = is_VMS(prob.method)
    

    stab_coeff = compute_stab_coeff(prob,params)
    Tm = momentum_stabilization(u_adv, stab_coeff, params)
    Tc = continuity_stabilization(u_adv, stab_coeff, params)
  
    ### VMS extra equations
    Auu_vms1(u, v) = vms_activation.* ∫(u_adv ⋅ (∇(v))'*Tm⊙ (cconv ∘ (u_adv, ∇(u))) )dΩ + vms_activation.*
    skewcoeff .*∫(u_adv ⋅ (∇(v))'*Tm⊙ (u_adv ⋅ (∇ ⋅ u)) )
